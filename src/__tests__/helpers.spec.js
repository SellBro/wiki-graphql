/* Unit Testing helpers functions */
...
describe('Display separate season number and episode number function:', () => {
  test('should return display number correctly with expected input', () => {
    expect(displaySeparateEpisode('S02E02')).toBe('S02 E02');
    expect(displaySeparateEpisode('S13E13')).toBe('S13 E13');
  });

  test('should return display number correctly with different length season and episode', () => {
    expect(displaySeparateEpisode('S1E2')).toBe('S1 E2');
...
describe('getEpisode function:', () => {
  ...
  test('should return episode: S01', () => {
    const expectedObj = {
      filterOptions: {
        name: '',
        episode: 'S01',
    ...
    expect(getEpisode(filter, { option: 'season', value: '1' })).toStrictEqual(expectedObj);
  });

  test('should return episode: E04', () => {
    const expectedObj = {
      filterOptions: {
        name: '',
        episode: 'E04',
    ...
    expect(getEpisode(filter, { option: 'episode', value: '4' })).toStrictEqual(expectedObj);
  });
});
